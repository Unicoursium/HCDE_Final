...
# === Initial Setup, if needed, just change the things below. ===
music_path = "allure"
...
pump_channels = [0, 1, 2, 3, 4, 15, 14, 13]  # pumps for each button
led_channels =  [5, 6, 7, 8, 9, 10, 11, 12]  # LED indicators
...
def set_pwm(channel, value):
    duty = int(value / 100 * 0xFFFF)
    pca.channels[channel].duty_cycle = duty

def turn_on_led(i):
    set_pwm(led_channels[i], 100)

def turn_off_led(i):
    set_pwm(led_channels[i], 0)

def turn_on_pump(i):
    set_pwm(pump_channels[i], 100)

def turn_off_pump(i):
    set_pwm(pump_channels[i], 0)
